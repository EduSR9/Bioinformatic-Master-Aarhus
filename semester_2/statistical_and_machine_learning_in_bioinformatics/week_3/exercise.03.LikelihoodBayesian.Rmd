...
```{r}
loglikelihood <- function(k, n){
  sampling <- c()
  for (teta in seq(0, 1, length.out = 1000)){
    likeli <- dbinom(k, n, teta, log = TRUE)
    sampling[teta] <- dbinom
    }
}
loglikelihood(40,300,40/300)
```
...
analysis. But you can also all $N$ simulations for a given
\theta interval, count how many simulations yielded a given $y_{obs}$
